...
using FEA.URVP.Application.Commands.Notifications.NotifyPlacementConfirmed;
...
public sealed class PlacementAssignedEventHandler : IEventHandler<PlacementAssignedEvent>
{
    private readonly IMediator _mediator;

    public PlacementAssignedEventHandler(IMediator mediator) => _mediator = mediator;

    public Task HandleAsync(PlacementAssignedEvent domainEvent, CancellationToken cancellationToken = default) =>
        _mediator.Send(new NotifyPlacementConfirmedCommand(domainEvent.PlacementId), cancellationToken);
}
