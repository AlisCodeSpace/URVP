...
public static class MimeTypes
{
    ...
    public readonly static FileType ZIP_7z = new FileType(new byte?[] { 66, 77 }, "7z", "application/x-7z-compressed");
    public readonly static FileType ZIP_7z_2 = new FileType(new byte?[] { 0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C }, "7z", "application/x-7z-compressed");

    public readonly static FileType ZIP = new FileType(new byte?[] { 0x50, 0x4B, 0x03, 0x04 }, "zip", "application/x-zip-compressed");
    public readonly static FileType RAR = new FileType(new byte?[] { 0x52, 0x61, 0x72, 0x21 }, "rar", "application/x-rar-compressed");
    ...
    public static void LoadFromXmlFile(string path)
    {
        ...
        {
            var serializer = new System.Xml.Serialization.XmlSerializer(types.GetType());
            List<FileType>? tmpTypes = serializer.Deserialize(file) as List<FileType>;
            if (tmpTypes == null)
            {
                return;
            }
        ...
        foreach (FileType type in types)
        {
            ...
            if (matchingCount == type.Header.Length)
            {
                ...
                if (type.Equals(ZIP) && !String.IsNullOrEmpty(fileFullName))
                    fileType = CheckForDocxAndXlsx(fileFullName) ?? type;
                else
    ...
    private static FileType? CheckForDocxAndXlsx(string fileFullName)
    {
    ...
    private static Byte[] ReadFileHeader(FileInfo file, int MaxHeaderSize)
    {
        ...
        try  // read file
        {
            ...
            {
                // Read up to MaxHeaderSize bytes and avoid inexact single Read call.
                int totalRead = 0;
                while (totalRead < MaxHeaderSize)
                {
                    int bytesRead = fsSource.Read(header, totalRead, MaxHeaderSize - totalRead);
                    if (bytesRead == 0)
                    {
                        break;
                    }
