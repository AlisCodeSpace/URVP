...
{
    public class ApproveChallengeCommandHandler : BaseCommandHandler<ApproveChallengeCommand, ChallengeDto>
    {
        private readonly IChallengeRepository _repository;
        private readonly IFileReadService _fileReadService;
        private readonly IEventBus _eventBus;
        public ApproveChallengeCommandHandler(
            IChallengeRepository repository,
            IFileReadService fileReadService,
            IEventBus eventBus,
            ILogger<ApproveChallengeCommandHandler> logger,
        ...
        protected override async Task<ChallengeDto> HandleInternal(ApproveChallengeCommand command, CancellationToken cancellationToken)
        {
            // Validation is handled by ValidationBehavior and ApproveChallengeCommandValidator
            ...
            var researchField = await _context.ResearchFields.FirstOrDefaultAsync(rf => rf.Id == updatedChallenge.ResearchFieldId, cancellationToken);
            ...
            await _eventBus.PublishAsync(new ChallengeApprovedEvent(
                updatedChallenge.Id,
                command.AdminId,
                updatedChallenge.Title,
                adminUser?.Name ?? "Admin",
