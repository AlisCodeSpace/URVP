...
{
    /// <summary>
    /// Handler for ApproveEditRequestCommand
    /// </summary>
    public class ApproveEditRequestCommandHandler : BaseCommandHandler<ApproveEditRequestCommand, ChallengeEditRequestDto>
    {
        private readonly IChallengeEditRequestRepository _editRequestRepository;
        private readonly IEventBus _eventBus;

        public ApproveEditRequestCommandHandler(
            IChallengeEditRequestRepository editRequestRepository,
            IEventBus eventBus,
            ILogger<ApproveEditRequestCommandHandler> logger,
        ...
        // Enable transaction support for edit request approval
        protected override bool UseTransaction => true;

        protected override async Task<ChallengeEditRequestDto> HandleInternal(ApproveEditRequestCommand command, CancellationToken cancellationToken)
        {
            _logger.LogInformation("Handling ApproveEditRequestCommand for EditRequestId: {EditRequestId}, AdminId: {AdminId}", 
            ...
            {
                throw new InvalidOperationException("Only pending edit requests can be approved");
            }
            ...
            // 4. Publish ChallengeEditRequestApprovedEvent
            var approvedEvent = new ChallengeEditRequestApprovedEvent(
                editRequest.Id,
            ...
            _logger.LogInformation("Successfully approved edit request {EditRequestId} for challenge {ChallengeId}", 
