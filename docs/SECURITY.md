...
**Redirect URIs.** URVP uses Azure AD only (AUB staff/students via LDAP). There is no Azure B2C
registration. Add these redirect URIs on the app registration whose `ClientId` is in
...
| Variable | Notes |
| --- | --- |
| `ASPNETCORE_ENVIRONMENT` | Must be `Staging` or `Production`. Set at the IIS site in `applicationHost.config` (the deploy script uses `/commit:apphost`) so a wipe-and-unzip of the site folder cannot drop it. Do not rely on a value only in `web.config`. |
...
| `AllowedHosts` | Real hostnames, semicolon-separated. No wildcard. Staging is `urvp-staging.aub.edu.lb`; Production is `urvp.aub.edu.lb;www.urvp.aub.edu.lb` |
...
3. `ASPNETCORE_ENVIRONMENT` is written by `scripts/iis/Set-AspNetCoreEnvironment.ps1` into
   ...
   revert the site to Development.
...
- Reproducible installs: `npm ci`, and `dotnet restore --locked-mode`.
